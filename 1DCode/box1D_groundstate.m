...
distance = .7*waist;
boxsizex = 15;
offsetx = rem(boxsizex, distance);
centers = generate_centers2D(-boxsizex + offsetx, 0, boxsizex, 1, distance);
...
nuaxial = 0.05;
height = mRb*boxsizex^2*(2*pi*nuaxial)^2/2; % height of the potential peak
...
Physics1D = Potential_Var1d(Method, Physics1D, ...
    @(x) 0.5*mRb*(2*pi*nuaxial)^2*x.^2/hbar + height*avgpot1D(x, waist, centers)/hbar); % harmonic ground state
% avgpot1D will normalise the spots for overlap
